...
@section('content')
    ...
    {{-- <div class="text-right">
        <a class="btn btn-success btn-sm mr-auto" href="{{route('stuff-attend.create')}}">新規登録</a>
    </div> --}}

    <!-- 検索エリア -->
    <form  class="my-2 my-lg-0 ml-2" action="{{route('stuff-attendserch')}}" method="GET">
        @csrf
        @method('get')
        <table>
            <div style="display:inline-flex">
                <div class="col-sm-4">
                    <a class="btn btn-success btn-sm" href="{{route('stuff-attend.create')}}">新規</a>
                </div>
                <div class="col-sm-8">
                ...
                {{-- <div class="col-sm-4">
                    <a class="btn btn-secondary btn-sm ml-2" href="{{route('stuff-attendexport')}}">CSV出力</a>
                </div> --}}
            </div>
    ...
    <form  class="my-2 my-lg-0 ml-2" action="{{route('stuff-attendexport')}}" method="GET">
        @csrf
        @method('get')
        <table>
            <div style="display:inline-flex">
    ...
    <div class="table-responsive">
        ...
        <table class="table table-striped table-borderd table_sticky">
            <thead>
                <tr>
                    ...
                    <th scope="col" class ="fixed01">@sortablelink('eventdate', '出勤日')</th>
            ...
            <tbody>
                @if($stuffattendances->count())
                    @foreach($stuffattendances as $stuffattend)
                    <tr>
                        {{-- ID --}}
                        <td>{{ $stuffattend->id }}</td>

                        {{-- 生徒名 --}}
                        @php
                            $fullname = $stuffattend->last_name.' '.$stuffattend->first_name
                        @endphp
                        <td>{{ $fullname }}</td>
                        ...
                        @foreach ($loop_atd_status_stuff as $loop_atd_status_stuff2)
                            @if ($loop_atd_status_stuff2['no']==$stuffattend->status)
                                <td>{{ $loop_atd_status_stuff2['name'] }}</td>
                            @endif
                        @endforeach
                        ...
                        <td>
                            <div class="btn-toolbar">
                                ...
                                <a class="btn btn-primary btn-sm" href="{{ route('stuff-attend.edit',$stuffattend->id)}}">編集</a>
                                </div>
                                <div class="btn-group me-2 mb-0">
                                    <form action="{{ route('stuff-attend.destroy', $stuffattend->id)}}" method="POST">
                                            @csrf
                                            @method('DELETE')
     ...
     <ul class="pagination justify-content-center">
        {{ $stuffattendances->appends(request()->query())->render() }}
     </ul>
